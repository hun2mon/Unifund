...
.stop_btn {
	...
	display: block; /* Ensure it takes up a block-level space */
...
<body>
	...
	<div class="container">
		...
		<c:forEach items="${adminMemberDetail }" var="adminMemberDetail">
			...
			<input type="hidden" value="${adminMemberDetail.mng_rsndate }" name="mng_getrsndate" class="mng_getrsndate"/>
...
function openModal(btn) {
	...
	var rsndate = $('.mng_getrsndate').val();
	...
	console.log("눌리니? ");
	    ...
	    if (btn === 'stopRequest') {
		    ...
		    modal += '<option value="90">90알</option>';
		    modal += '<option value="영구정지">영구정지</option>';
		    modal += '</select>';
		    modal += '</td>';
		    modal += '</tr>';
		    modal += '</table>';
		    ...
		    modal += '</tr>';
		    modal += '<tr>';
		    modal += '<td><input type="text" class="mng_rsndate" name="mng_rsndate" value="'+rsndate+'"/></td>';
		    modal += '</tr>';
		    modal += '<tr>';
		    modal += '<th>정지일수</th>';
		    modal += '</tr>';
		    modal += '<tr>';
		    modal += '<td><input type="text" class="msg_stopdate" name="msg_stopdate" value="'+stopdate+'"/></td>';
		    modal += '</tr>';
		    modal += '<tr>';
		    modal += '<th>정지일수 변경</th>';
		    modal += '</tr>';
		    ...
		    modal += '<td>';
		    modal += '<select class="stopDay" name="msg_stopdate" id="stopDay">';
		    modal += '<option value="0">정지해</option>';
		    modal += '<option value="1">1일</option>';
		    modal += '<option value="7">7일</option>';
		    modal += '<option value="14">14일</option>';
		    modal += '<option value="30">30일</option>';
		    ...
		    modal += '</table>';
	        modal += '<input type="button" id="cancel" class="cancel" value="취소"> ';
		    modal += '<input type="button" id="ok" class="ok" value="변경"> ';
	    }
	    modal += '</div> ';
	    modal += '</div>';
